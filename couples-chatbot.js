...
const getMongoSession = require("./services/sessionManager");
const { addMessage, getMessages, deleteMessages, saveSession, getSession, deleteSession } = require("./services/redisSession");
...
app.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    Use /create to start a couples session.`
      ...
      if (!/^[A-F0-9]{8}$/.test(sessionCode)) {
        await sendTelegramMessage(chatId, "Invalid session code.");
        return res.sendStatus(200);
      }

      // 🔹 Try Redis first

      let redisSession = await getSession(sessionCode);

      // 🔹 Fallback to Mongo if not in Redis
      if (!redisSession) {
        const mongoSession = await Session.findOne({ sessionCode });
        ...
        redisSession = mongoSession.toObject();

        if (redisSession && !redisSession.active) {
          await sendTelegramMessage(chatId, "Session is no longer active.");
          return res.sendStatus(200);
        }

        // hydrate Redis
        await saveSession({
          sessionCode: redisSession.sessionCode,
          partner1: redisSession.partner1,
          partner2: redisSession.partner2,
          active: redisSession.active
        });
      }

      // ❗ Prevent same user joining twice
      if (
        redisSession.partner1 === userId ||
        redisSession.partner2 === userId
      ) {
        await sendTelegramMessage(chatId, "You are already in this session.");
        return res.sendStatus(200);
      }

      // ❗ Assign partner1 if empty
      if (!redisSession.partner1) {
        redisSession.partner1 = userId;
      }
      // ❗ Assign partner2
      else if (!redisSession.partner2) {
        redisSession.partner2 = userId;
      }
      // ❗ Session full
      else {
        await sendTelegramMessage(chatId, "Session already full.");
        return res.sendStatus(200);
      }
    ...
    if (command === "/status") {
      ...
      const session = await getMongoSession(userId);
    ...
    if (command === "/leave") {
      ...
      // 🗑 Delete messages (MongoDB)
      await Message.deleteMany({ sessionId: session.sessionCode });

      // 🔴 Deactivate session (MongoDB)
      session.active = false;
      await session.save();

      // ⚡ Remove from Redis
      await deleteMessages(session.sessionCode); // 🔥 delete chat
      await deleteSession(session.sessionCode);  // 🔥 delete session

      await sendTelegramMessage(
        chatId,
        "Session ended. Data cleared."
      );
  ...
  // }
    ...
    const aiPrompt = buildMediatorSystemPrompt({
      ...
      partnerName: session[otherPartnerId] ? "Partner" : "Unknown"
    });
    ...
    await addMessage(session.sessionCode, {
      sender: userId,
      content: messageText,
      role: "user"
    });
    ...
    // THEN fetch history
    // let history = await getMessages(session.sessionCode);
    ...
    // 🔁 fallback to Mongo if Redis empty
    if (!history.length) {

      const dbHistory = await getConversationHistory(session.sessionCode);
